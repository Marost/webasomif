...
$pagina="Normas Legales";
	...
	//DOCUMENTOS Y VIDEOS
	$rst_query2=mysqli_query($conexion, "SELECT * FROM ap_evaluacion WHERE id>0 AND publicar=1 ORDER BY id DESC LIMIT 5");
	...
	$rst_query2=mysqli_query($conexion, "SELECT * FROM ap_evaluacion WHERE id>0 AND publicar=1 ORDER BY id DESC LIMIT $inicio, $registros;");
	$paginas=ceil($num_registros/$registros);
		
	//DOCUMENTOS VIDEOS
	//$rst_query2=mysqli_query($conexion, "SELECT * FROM ap_evaluacion WHERE id>0 AND tipo='Documento' AND publicar=1 ORDER BY id DESC LIMIT 4 ");
	//$rst_query3=mysqli_query($conexion, "SELECT * FROM ap_evaluacion WHERE id>0 AND tipo='Video' AND publicar=1 ORDER BY id DESC LIMIT 4 ");
...
<title>Intranet - Normas Legales</title>
...
<div id="interior-sombra">
              ...
              <div id="contenido525">
              <div id="div-ancho100">
                <p class="texto-negro15-MyriadProBold">NORMAS LEGALES</p>
                <p class="texto-negro11-Arial">Contenido publicado en el Diario Oficial El Peruano relacionado a temas que interesan a nuestro sector. La publicación es de carácter informativo, tomada de fuentes confiables. Ante cualquier duda, consulte con la fuente original. Realizamos el máximo esfuerzo por tener esta sección actualizada.</p>
                <p class="texto-azul14-MyriadProBold"><a href="http://www.elperuano.pe" target="_blank"> Enlace directo al Diario El Peruano</a></p>
                <p class="texto-negro15-MyriadProBold">&nbsp;</p>
                </div>
                    ...
                    <?php }elseif($fila_query2["tipo"]=='Video'){ ?>
                      <div id="documentos-pdf-doc">
                                <div id="icon-pdf-doc">
                                    ...
                                    <form action="conexion/guardarDescarga.php?tipo_pagina=normas_legales" method="post" id="video<?php echo $fila_query2["id"]; ?>" onsubmit="abrirVentana('video.php?id=<?php echo $fila_query2["id"]; ?>&tipo_pagina=normas_legales','video','width=640,height=385')">>

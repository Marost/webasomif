...
$buscar=$_REQUEST["busqueda"];
	...
	if ($_REQUEST["btnbuscar"]=="")
	{
		...
		$rst_query=mysql_query("SELECT * FROM ap_foro WHERE id>0 ORDER BY foro ASC LIMIT $inicio, $registros;", $conexion);
	...
	{
		$rst_query=mysql_query("SELECT * FROM ap_foro WHERE foro LIKE '%$buscar%' ORDER BY foro ASC;", $conexion);
		$num_registros=mysql_num_rows($rst_query);
		...
		$rst_query=mysql_query("SELECT * FROM ap_foro WHERE foro LIKE '%$buscar%' ORDER BY foro ASC LIMIT $inicio, $registros;", $conexion);
		$paginas=ceil($num_registros/$registros);

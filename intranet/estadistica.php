...
$pagina="Estadística";
	...
	//DOCUMENTOS Y VIDEOS
	$rst_query2=mysqli_query($conexion, "SELECT * FROM ap_estadistica WHERE id>0 AND publicar=1 ORDER BY id DESC LIMIT 5");
	...
	$rst_query2=mysqli_query($conexion, "SELECT * FROM ap_estadistica WHERE id>0 AND publicar=1 ORDER BY id DESC LIMIT $inicio, $registros;");
	$paginas=ceil($num_registros/$registros);
		
	//DOCUMENTOS VIDEOS
	//$rst_query2=mysqli_query($conexion, "SELECT * FROM ap_estadistica WHERE id>0 AND tipo='Documento' AND publicar=1 ORDER BY id DESC LIMIT 4");
	//$rst_query3=mysqli_query($conexion, "SELECT * FROM ap_estadistica WHERE id>0 AND tipo='Video' AND publicar=1 ORDER BY id DESC LIMIT 4");
...
<title>Intranet - Estadísticias</title>
...
<div id="interior-sombra">
              ...
              <div id="contenido525">
              <div id="div-ancho100">
                <p class="texto-negro15-MyriadProBold">ESTADÍSTICAS</p>
                <p class="texto-negro11-Arial">Reporte mensual del Sistema de Microfinanzas del Perú. Estados financieros, Indicadores financieros, Cuadros comparativos, evolución de las colocaciones netas y participación de la cartera MES por IMF.</p>
                <p>&nbsp;</p>
                </div>
                    ...
                    <?php }elseif($fila_query2["tipo"]=='Video'){ ?>
                      <div id="documentos-pdf-doc">
                                <div id="icon-pdf-doc">
                                    ...
                                    <form action="conexion/guardarDescarga.php?tipo_pagina=estadisticas" method="post" id="video<?php echo $fila_query2["id"]; ?>" onsubmit="abrirVentana('video.php?id=<?php echo $fila_query2["id"]; ?>&tipo_pagina=estadisticas','video','width=640,height=385')">>
